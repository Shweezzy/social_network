...
router.put('/change_user_data/:user_data', auth, [
    ...
    async (req, res) => {
        try {
            ...
            if (user[userDataToChange] === changeData.toString()) return res.status(401).json('The given name is already in the database');
...
//зміна пароля
router.put('/check_actual_password', auth,
    [
        check('passwordToCheck', 'Password has to be 6 letter and below 12').isLength({
            min: 6,
            max: 12
        }),
    ],
...
router.put('/change_password', auth, [
        check('newPassword', 'New password must be between 6 and 12 characters long').isLength({
            min: 6,
            max: 12
        })
    ],
